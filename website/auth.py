...
def sign_up():
    ...
    if request.method == 'POST':
        ...
        if len(email) < 4:
            flash('invalid email')
        elif len(first_name) < 2 | len(last_name) < 2:
            ...
            if user:
                ...
                new_user = User(email=email, first_name=first_name, password=generate_password_hash(password, method='pbkdf2:sha256', salt_length=16))
